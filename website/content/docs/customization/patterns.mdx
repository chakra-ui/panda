...
For example, `@pandacss/preset-base` sets `gap: '8px'` on [`stack`](/docs/concepts/patterns#stack). To default to a
...
Values can be token keys (`'2'`) or literals (`'12px'`). `stack`, `hstack`, and `vstack` are separate patterns — override
