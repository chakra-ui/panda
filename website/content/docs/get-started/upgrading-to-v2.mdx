...
`@pandacss/is-valid-prop`, `@pandacss/logger`, `@pandacss/reporter`, and the Astro `@pandacss/studio`. The same goes
for `@pandacss/plugin-vue` and `@pandacss/plugin-svelte`: the compiler reads `.vue`, `.svelte`, and `.astro` files
directly, so remove them from `plugins`. If you only use `@pandacss/dev` plus Vite or PostCSS, you're fine.
...
- **Linting on the v2 engine.** The [ESLint & oxlint plugin](/docs/get-started/linting) lints against the
