...
See [The compiler engine](/docs/compiler/compiler-engine) for the pipeline, and
[How Panda works](/docs/compiler/how-panda-works) for the model it keeps.
...
- **Linting on the v2 engine.** The [ESLint & oxlint plugin](/docs/get-started/eslint-oxlint-plugin) lints against the
...
- **Studio.** The Astro-based `@pandacss/studio` is gone. A lighter, CLI-generated studio is planned. See
  [Studio in v2](/docs/tooling/studio-v2).
...
- [The compiler engine](/docs/compiler/compiler-engine) — the Rust pipeline underneath v2.
- [Config](/docs/reference/config) — the full v2 config reference.
- [CLI](/docs/reference/cli) — every command and flag.
- [How Panda works](/docs/compiler/how-panda-works) — the mental model v2 keeps.
