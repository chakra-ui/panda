...
Turn on [`strictTokens`](/docs/styling/writing-styles#stricttokens) to reject raw values too, so a color has to be a token, not a
one-off like `#f00`.
...
`tailwind-variants`. Panda has this built in as [recipes](/docs/recipes/atomic-recipe):
...
[Recipes](/docs/recipes/atomic-recipe) and [Slot Recipes](/docs/recipes/slot-recipes) for multi-part components.
...
[recipe](/docs/recipes/atomic-recipe) or the [`cx`](/docs/styling/merging-styles) helper for the same composition instead.
Tailwind plugins, loaded with `@plugin` in v4 (`@tailwindcss/typography`, `@tailwindcss/forms`) don't carry over
either, check [Presets](/docs/theming/presets) for the equivalent idea, a shareable package of tokens,
...
- [Migration strategy](/docs/get-started/migration-strategy) for running both tools side by side during the migration.
- [Writing Styles](/docs/styling/writing-styles) and [Recipes](/docs/recipes/atomic-recipe) for the two APIs this guide
  leans on most.
