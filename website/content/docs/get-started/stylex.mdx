...
Panda's [recipes](/docs/recipes/recipes) formalize this as a typed variant map, the prop and its allowed values are
...
See [Recipes](/docs/recipes/recipes) and [Slot Recipes](/docs/recipes/slot-recipes) for multi-part components.
...
scope. Panda ships [patterns](/docs/recipes/patterns) for common layout shapes, as both a function and a JSX
component:
...
- [Migration strategy](/docs/get-started/migration-strategy) for running both side by side during the migration.
