...
This setup resolves `css()` calls with a small runtime in the browser. Panda's webpack plugin (via Rspack's webpack-compatible API) can rewrite them to plain
