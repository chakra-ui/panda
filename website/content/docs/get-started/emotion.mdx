...
Panda's [recipes](/docs/recipes/recipes) replace this with a typed, declarative variant map, no manual prop-to-style
branching:
...
equivalent safety net. See [Recipes](/docs/recipes/recipes) and [Slot Recipes](/docs/recipes/slot-recipes).
...
a pre-built `Box` or `Stack`. Panda ships [patterns](/docs/recipes/patterns) for common layout shapes as both a
