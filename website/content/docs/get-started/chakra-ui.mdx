...
Panda's equivalent is a [recipe](/docs/recipes/recipes), either colocated with `cva`/`sva` or shared globally via
...
See [Recipes](/docs/recipes/recipes) and [Slot Recipes](/docs/recipes/slot-recipes).
...
Panda's [patterns](/docs/recipes/patterns) cover the same layout primitives, as a JSX component or a plain function:
