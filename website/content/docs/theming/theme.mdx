...
A token is a named value, `brand` instead of `#EA8433`. Panda groups them into categories like colors, spacing,
and radii, each shown below with its defaults. See [Tokens](/docs/theming/tokens) for the full token format.
