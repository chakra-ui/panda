...
Run `panda codegen` (or a full `panda` build) and Panda writes `styled-system/specs/tokens.json`. The other files below
describe the wider shape this format is moving toward — today only `tokens.json` is emitted.
