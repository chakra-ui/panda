...
- **A design system package.** One npm package that owns your tokens and theme.
- **Multiple apps, one look.** The same colors, spacing, and breakpoints everywhere, defined once.
- **A component library.** Consumers add your preset to their own `panda.config.ts`. See the
  [component library guide](/docs/design-systems/overview).

If the consuming app doesn't use Panda at all, a preset can't help, ship static CSS instead. The
[component library guide](/docs/design-systems/overview) covers that path too.
...
- [`staticCss`](/docs/compiler/static)
- [`theme`](/docs/theming/theme)
- [`utilities`](/docs/theming/utilities)

For how a whole component library gets built and shipped, not just its preset, see
[shipping styles](/docs/design-systems/ship-styled-system).
