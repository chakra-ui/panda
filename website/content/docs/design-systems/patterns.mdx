...
You can extend the [default patterns](/docs/styling/patterns#predefined-patterns) by using the `patterns.extend`
...
Use `defaultValues` in `patterns.extend` to change what a built-in pattern applies when you omit a prop. The shape is
the same for every pattern; only the pattern name and keys change.

For example, `@pandacss/preset-base` sets `gap: '8px'` on [`stack`](/docs/styling/patterns#stack). To default to a
...
Values can be token keys (`'2'`) or literals (`'12px'`). `stack`, `hstack`, and `vstack` are separate patterns, so override
each if you want the same default gap everywhere.
