...
> Note: Using this means you can't use the JS function [`token.var`](/docs/concepts/dynamic-styling#using-tokenvar) (or
> [token(xxx)](/docs/concepts/dynamic-styling#using-token) where `xxx` is the path to a
