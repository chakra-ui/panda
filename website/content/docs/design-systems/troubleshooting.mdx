...
manifest already covers. Remove it too. See [Monorepo workflow](/docs/design-systems/monorepo-workflow).
...
## The spec will not publish

```
design_system_spec_not_publishable
```

`panda lib --spec=<file>` wrote the spec somewhere `package.json` `"files"` would drop, so the manifest leaves it out.
Use a bare `--spec` to write it beside `preset.mjs`, or add the path to `"files"`.
