...
description: Run multiple Panda-built apps, or multiple versions of the same design system, on one host page without class-name or CSS-variable collisions.
---

Two independently-built Panda apps loaded onto the same page can emit the same class name, or the same CSS variable,
with different rules behind them. The cascade only keeps one. `prefix` and `cssVarRoot` are the config options that
stop that from happening.

This guide sets up a Panda lib so it can co-exist with other Panda libs, or with a different version of itself,
...
A hardcoded prefix drifts out of sync the moment you forget to bump it. Derive it from your package's major version
instead:
...
- [Component Library](/docs/design-systems/setup): distribution
  patterns for shipping a Panda lib
- [Presets](/docs/theming/presets): sharing tokens and recipes
  across consumers
