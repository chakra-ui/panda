...
library](/docs/design-systems/overview): one package owns the design system, everyone else imports from it.
...
An upcoming version of Panda is expected to collapse the `presets` plus `importMap` pairing above into a single
`designSystem` field that resolves both automatically. See [Consuming a design
system](/docs/design-systems/consuming-a-design-system) for what that looks like and how stable it is right now, this
page's `presets` and `importMap` combination is what actually ships and works today.
