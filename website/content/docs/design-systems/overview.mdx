...
1. Ship a Panda [preset](/docs/design-systems/presets)
...
them in your projects. See the [end-to-end preset example](/docs/design-systems/presets#end-to-end-example) for the full
...
Finally, don't forget to include the [cascade layers](/docs/styling/cascade-layers) as well in your app code:
...
- You might need add the [prefix](/docs/reference/config#prefix) option to avoid className conflicts
...
- create a Panda [preset](/docs/design-systems/presets) so that you (and your users) can share the same design system
  tokens
...
3. Run the `panda init` command in there to generate a `panda.config.ts` file, don't forget to set the `jsxFramework` if
   needed
4. [optional] you might want to install and import your [preset](/docs/design-systems/presets) in this `panda.config.ts`
   file as well
5. Run the [`panda emit-pkg`](/docs/reference/cli#emit-pkg) command to set the entrypoints in
