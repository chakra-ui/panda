...
A design system here is one package that ships both your components and the Panda config they were built from, tokens,
recipes, all of it. Apps that consume it don't re-extract your source. They load a small manifest you publish alongside
the package.
...
<Callout type="info">
  If your apps do not run Panda, ship a stylesheet instead: [Consume without
  Panda](/docs/design-systems/consume-without-panda). If you only need tokens, without components, ship a
  [preset](/docs/theming/presets#creating-a-preset).
</Callout>
...
Inside the package, `Button` imports from the local `styled-system`. Once published, consumers of this walkthrough just
...
```ts filename="packages/marketing/panda.config.ts"
import { defineConfig } from '@pandacss/dev'
...
See [Nested design systems](/docs/design-systems/consume-with-panda#nested-design-systems) for the app config.
...
Bundler setup, imports, and theme overrides: [Consume with Panda](/docs/design-systems/consume-with-panda).
