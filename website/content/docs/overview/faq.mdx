...
At build time, Panda parses your source with a Rust and Oxc engine, extracts the styles you use, and emits the CSS
natively — one parse per file, no PostCSS in the hot path. It also generates a small runtime and types in your
`styled-system` folder.

For the mental model and the full pipeline, see [thinking in Panda](/docs/overview/thinking-in-panda).
