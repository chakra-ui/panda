...
- **Build-time extraction:** Panda parses your source with a Rust and Oxc engine, reads the styles you use, and
  generates atomic CSS — one parse per file, no styles computed in the browser. See
  [how Panda works](/docs/overview/how-panda-works) for the full pipeline.
