...
Report which tokens, recipes, utilities, patterns, and keyframes your project uses, and which configured ones no
scanned file touches. It scans the whole project, following your `include` and `exclude` options, and prints a
summary followed by one section per category. For what to do with the report, see
[Find what nobody uses](/docs/design-systems/analyze).
