...
`scrollbarColor` is the escape hatch for a raw `thumb track` pair. Don't use it for a single color token. That's invalid
CSS. Use `scrollbarThumb` / `scrollbarTrack`.
...
<UtilityTable groups={['Interactivity', 'Scroll']} />
