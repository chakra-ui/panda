...
Whether to minify the generated css. This can be set to `true` to reduce the size of the generated css. Usually
...
CSS emission optimizations. Every optimization is opt-in. See
[Optimization](/docs/styling/optimization) for when to turn each flag on.
