...
Pass `--zip` to write the same files as a single `debug.zip` instead, which is easier to attach to a bug report.
...
recipes, or that options like `hash` and `minify` resolved the way you expect for the current `NODE_ENV`.
...
This writes a `.panda/trace.json` file — open it in `chrome://tracing` or [ui.perfetto.dev](https://ui.perfetto.dev) —
along with a `.panda/timings.json` summary.
...
included. [If you're not using `postcss`](/docs/get-started/cli), ensure that `styled-system/styles.css` is imported and
that the `panda` command has been run (or is running with `--watch`).
...
In those rare cases, you might want to swap to using the [CLI](/docs/get-started/cli) instead, as it will always be more
performant than the postcss alternative since we directly watch for filesystem changes and only run the extract/codegen
steps when needed.
