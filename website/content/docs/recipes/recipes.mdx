...
[Comparison table between the different types of recipes here: "Should I use atomic or config recipes ?"](/docs/recipes/recipes#should-i-use-atomic-or-config-recipes-)
...
[static CSS](/docs/compiler/static#generating-recipes).

### Using compound variants

Apply styles when **multiple** variant props match. Same shape as [`cva`](/docs/recipes/recipes#compound-variants): a
...
Runtime-only combos need [`staticCss`](/docs/compiler/static#generating-recipes). Run `pnpm panda codegen` after editing
the recipe.
...
- The recipe is defined in config with `defineRecipe`, not [`cva`](/docs/recipes/recipes#atomic-recipe-or-cva)
...
- [`cva`](/docs/recipes/recipes#atomic-recipe-or-cva): no responsive variant props. Config recipes: yes, unless
  `compoundVariants` is set (see [above](/docs/recipes/recipes#using-compound-variants)).
...
Use `staticCss` to pre-generate recipe CSS Panda can't extract, including [dynamic variant props](/docs/recipes/recipes#dynamic-variant-props). See [Generating recipes](/docs/compiler/static#generating-recipes).

## Should I use atomic or config recipes ?

[Config recipes](/docs/recipes/recipes#config-recipe) are JIT: Panda only emits variants it finds in your code.
[`cva`](/docs/recipes/recipes#atomic-recipe-or-cva) recipes live anywhere in the app, so every variant is generated up
front.
