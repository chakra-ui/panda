...
const Button = (props: ButtonProps) => {
  ...
  return (
    <button {...rest} className={cx(button({ size }), className)}>
      {children}
    </button>
  )
}
...
Two things to keep in the wrapper: pull the variant props out of `props` so they don't land on the DOM element, and
merge any incoming `className` with [`cx`](/docs/styling/writing-styles#classname-concatenation) so callers can still
...
- Due to static nature of Panda, it's not possible to track the usage of the recipes in all cases. Here are some of use
  ...
  **When you change the name of the variant prop in the JSX component**
  ...
  ```tsx
  const Button = ({ buttonSize, className, children, ...props }) => {
    return (
      <button {...props} className={cx(button({ size: buttonSize }), className)}>
        {children}
      </button>
    )
  }
