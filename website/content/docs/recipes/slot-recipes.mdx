...
`base` and each variant option are **per slot**, not a flat style object. There is no `compoundSlotVariants`.
Compounds still use `compoundVariants`, with `css` written per slot.
...
To wrap a tree of parts (Ark, Radix, Base UI), use [Slot recipe context](/docs/recipes/slot-recipe-context).
