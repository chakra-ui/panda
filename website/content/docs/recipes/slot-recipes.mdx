...
[See the comparison table between atomic recipes (`cva`) and `config recipes` here.](/docs/recipes/recipes#should-i-use-atomic-or-config-recipes-)
...
[JSX Style Context](/docs/recipes/jsx-style-context).
...
Same rules as [config recipes](/docs/recipes/recipes#dynamic-variant-props). The runtime function returns slot class
