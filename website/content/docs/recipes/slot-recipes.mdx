...
> [Chakra UI](https://chakra-ui.com/docs/theming/slot-recipes) and slot variants in
> [Tailwind Variants](https://tailwind-variants.org)

[See the comparison table between atomic recipes (`cva`) and `config recipes` here.](/docs/recipes/config-recipe#should-i-use-atomic-or-config-recipes-)
...
Same rules as [config recipes](/docs/recipes/config-recipe#dynamic-variant-props). The runtime function returns slot class
...
function Media({
  ...
  return (
    <ul className={cx(classes.container)}>
      {items.map((item) => (
...
Register as `theme.slotRecipes.mediaGrid`, then import `mediaGrid` and `MediaGridVariant` from `<outdir>/recipes`.
...
// root    → "tabs__root tabs__root--size_sm"
// trigger → "tabs__trigger tabs__trigger--active_true text_blue.500"
```
...
```tsx
const { root } = accordion({ size: { base: 'sm', md: 'lg' } }) // ✅
```
...
// ❌ [recipe:tabs__trigger:size] Conditions are not supported when using compound variants.
```
...
[`sva`](/docs/recipes/slot-recipes#compound-variants), all compound combos ship as atomic classes when the recipe is
defined.
