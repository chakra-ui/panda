...
The MCP server is distributed as `@pandacss/mcp`. Run it directly with `npx`, `pnpm dlx`, or your package manager's
equivalent one-off package runner.
...
When you run `@pandacss/mcp init`, the following configuration is generated for each selected client:
