...
MCP ships as a separate package so `@pandacss/dev` installs do not pull the MCP SDK (and Hono) into every project.

> `panda mcp` / `panda init-mcp` from `@pandacss/dev` now exit with a migration error. Use `@pandacss/mcp` directly.
...
npx -y @pandacss/mcp init --client claude,cursor

# Specify working directory
npx -y @pandacss/mcp init --cwd ./my-project
```
...
npx -y @pandacss/mcp --config ./panda.config.ts

# Specify working directory
npx -y @pandacss/mcp --cwd ./my-project
```
...
When you run `npx -y @pandacss/mcp init`, the following configuration is generated for each selected client:
