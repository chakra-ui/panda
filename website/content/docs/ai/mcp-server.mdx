...
> Prefer running the MCP package directly with `npx -y @pandacss/mcp` (or `pnpm dlx @pandacss/mcp`).
> `panda mcp` / `panda init-mcp` still work and shell out to the same package.
...
3. Check that `npx -y @pandacss/mcp` (or `npx panda mcp`) runs without errors
