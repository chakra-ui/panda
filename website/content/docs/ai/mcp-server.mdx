...
The MCP server is distributed as `@pandacss/mcp`. It is optional and separate from `@pandacss/dev`, so normal Panda
installs do not pull in MCP dependencies.
...
`panda init-mcp` still works as a compatibility shim and shells out to the same command.
...
pnpm dlx @pandacss/mcp init --client claude,cursor

# Specify working directory
pnpm dlx @pandacss/mcp init --cwd ./my-project
```
...
When you run `pnpm dlx @pandacss/mcp init`, the following configuration is generated for each selected client:
...
1. Ensure you can run `pnpm dlx @pandacss/mcp --help`
2. Verify you have a valid `panda.config.ts` in your project
3. Check that `npx -y @pandacss/mcp` runs without errors

### Migrating from `panda mcp`

Older configs used `["panda", "mcp"]`. Update them to `["-y", "@pandacss/mcp"]`, or re-run
`pnpm dlx @pandacss/mcp init`.

`panda mcp` and `panda init-mcp` remain as compatibility shims that shell out to `@pandacss/mcp`.
