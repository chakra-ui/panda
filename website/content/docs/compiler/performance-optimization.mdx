...
Panda ships zero runtime cost, see [How Panda works](/docs/compiler/how-panda-works), but that doesn't mean nothing
...
it with a [`config:resolved` hook](/docs/theming/hooks) that filters which properties get the strict
...
- [How Panda works](/docs/compiler/how-panda-works) for why there's no runtime cost to weigh against these build/IDE
  costs in the first place.
- [Debugging](/docs/reference/debugging) for `--profile`, `panda debug`, and `PANDA_DEBUG`.
- [Static CSS Generation](/docs/compiler/static) and [Federated Micro-Frontends](/docs/design-systems/federated-microfrontends)
