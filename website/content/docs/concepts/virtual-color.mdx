...
See [using a custom property](/docs/concepts/color-opacity-modifier#using-a-custom-property) for the
`{colors.colorPalette…/opacity}` syntax on `--*` variables:
...
> Learn more about [static css generation](/docs/distribution/static).
