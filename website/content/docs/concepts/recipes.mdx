...
;<input className={input({ size: 'sm', variant: 'outline' })} />
```

First extracted use emits every compound class in `@layer recipes.compound_variants`:

```css
@layer recipes.compound_variants {
  .input--compound__size_sm__variant_outline {
    padding-inline: var(--spacing-2);
    padding-block: var(--spacing-1);
  }
  .input--compound__size_md__variant_outline {
...
By default, first use emits CSS for every compound entry. Narrow that with:
...
[dynamic variant props](/docs/concepts/recipes#dynamic-variant-props). See
[Generating recipes](/docs/guides/static#generating-recipes).
