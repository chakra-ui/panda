...
[static CSS](/docs/distribution/static#generating-recipes).
...
Runtime-only combos need [`staticCss`](/docs/distribution/static#generating-recipes). Run `pnpm panda codegen` after editing
the recipe.
...
Use `staticCss` to pre-generate recipe CSS Panda can't extract — including [dynamic variant props](/docs/concepts/recipes#dynamic-variant-props). See [Generating recipes](/docs/distribution/static#generating-recipes).
