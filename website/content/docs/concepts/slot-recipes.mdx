...
Same rules as [config recipes](/docs/concepts/recipes#dynamic-variant-props). The runtime function returns slot class
...
Register as `theme.slotRecipes.mediaGrid` — import `mediaGrid` and `MediaGridVariant` from `<outdir>/recipes`.
...
// trigger → "tabs__trigger tabs__trigger--active_true tabs__trigger--compound__active_true__size_sm"
```

First extracted use emits compound classes on affected slots:

```css
@layer recipes.compound_variants {
  .tabs__trigger--compound__active_true__size_sm {
    color: var(--colors-blue-500);
  }
  .tabs__trigger--compound__active_true__size_md {
    color: var(--colors-red-500);
  }
}
```

Only slots listed in `css` get compound classes. Adding `compoundVariants` drops responsive variant props — compare
...
[`sva`](/docs/concepts/slot-recipes#compound-variants) is different — all compound combos ship as atomic classes when
the recipe is defined, not at config extraction time.
