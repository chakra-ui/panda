...
> Use [`hash`](/docs/references/config#hash) in production only — it makes debugging harder. See
> [Environment-specific config](/docs/distribution/environment-specific-config) for branching on `process.env`.
