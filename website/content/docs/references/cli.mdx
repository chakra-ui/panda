...
These commands were moved out of `@pandacss/dev` for security reasons so installs no longer pull the MCP SDK (and Hono).

They now exit with a migration error. Use the standalone package instead:

```bash
# Start the MCP server
npx -y @pandacss/mcp

# Initialize MCP config for AI clients
