...
Start the MCP (Model Context Protocol) server for AI assistants. This is a compatibility shim that shells out to
`npx -y @pandacss/mcp`. Prefer running `@pandacss/mcp` directly.
...
Initialize MCP configuration for AI clients. This is a compatibility shim that shells out to
`npx -y @pandacss/mcp init`. Prefer running `@pandacss/mcp` directly.
