...
Often enabled only in production — see [Environment-specific config](/docs/distribution/environment-specific-config).
...
Check out the [Component Library](/docs/component-library/overview) guide for more information on how to use the
`importMap` option.
...
[Hooks](/docs/customization/hooks) docs for more information.
