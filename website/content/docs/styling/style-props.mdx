...
> Same atomic CSS as `css()`, different authoring surface. For variants and compound components, step up to
> [recipes](/docs/recipes/atomic-recipe) or walk the ladder in
> [Thinking in Panda](/docs/get-started/thinking-in-panda).
...
are extracted at the call site, on props the compiler recognizes: CSS utilities (`width`, `padding`, …) when
...
One override → `css`. Named parts → `trackCss`, `slideCss`, etc. Other prop names → `css.raw()`. See
...
[you're tracking those components in your recipe config](/docs/recipes/config-recipe#advanced-jsx-tracking).
...
> _and_ pass it to your component, see [forwarding props](/docs/recipes/guide-forwarding-props) in
...
> Learn more about the [patterns](/docs/theming/patterns) we provide.
