...
In Panda, you write your styles as plain objects, right next to your markup, and Panda compiles them down to CSS at
build time. What you actually have to decide, over and over as a component grows, is which API to reach for next.

The rule of thumb is to start local, and only promote when the styles force you to. We'll build a Button, then a Card,
moving up the ladder each time: `css` → `cva` → `sva` → config.

Want the case for build-time styles first? Read [Why Panda](/docs/styling/why-panda).
...
- [Getting started](/docs/styling/getting-started): install Panda and generate your first styles.
