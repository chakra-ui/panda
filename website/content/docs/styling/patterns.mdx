...
> Patterns are for layout primitives (`stack`, `grid`, `container`). Component variants with named states belong in
> [recipes](/docs/recipes/atomic-recipe). See [Thinking in Panda](/docs/get-started/thinking-in-panda) for when to
> promote.

## Creating Patterns

To learn how to create patterns, check out the [customization](/docs/theming/patterns) section.
...
Change defaults in [config](/docs/theming/patterns#changing-a-patterns-defaults). `HStack` and `VStack` are separate
...
Change defaults in [config](/docs/theming/patterns#changing-a-patterns-defaults).
...
Learn more about pattern customization [here](/docs/theming/patterns).
...
- **Pattern props**: compiled by the pattern (`columns`, `colSpan`, `direction`, `ratio`, …)
- **CSS utilities**: passed straight to the style system (`padding`, `display`, `color`, …)
...
// ❌ pattern props inside `md` skip the pattern transform
<Grid columns={1} md={{ columns: 2 }} />
<Stack direction="column" md={{ direction: 'row' }} />
```

Pattern props are transformed at the top level (`columns` → `gridTemplateColumns`, `colSpan` → `gridColumn`, etc.).
Nesting them in `md={{ … }}` skips that step, so nothing changes at the breakpoint.

CSS utilities on the same component still work with breakpoint keys, same as on `Box` or `styled.div`:

```tsx
<Grid columns={{ base: 1, md: 2 }} padding={{ base: '4', md: '6' }} />
```

The same call-site rules apply to custom components: use `css`, `*Css`, or CSS utility names, not semantic aliases.
See [Extraction rules](/docs/styling/extraction-rules).
