...
included. [If you're not using `postcss`](/docs/compiler/cli), ensure that `styled-system/styles.css` is imported
...
You can get a [`config recipe`](/docs/recipes/recipes#config-recipe) properties types by using `XXXVariantProps`. Let's
...
You can get an [`atomic recipe`](/docs/recipes/recipes#atomic-recipe-or-cva) properties types by using `RecipeVariantProps`.
...
[Config recipes](/docs/recipes/recipes#config-recipe) are generated just in time, meaning that only the recipes and
variants you use will exist in the generated CSS, regardless of the number of recipes in the config.

This contrasts with [Atomic recipes](/docs/recipes/recipes#atomic-recipe-or-cva) (cva), which generates all of the variants
