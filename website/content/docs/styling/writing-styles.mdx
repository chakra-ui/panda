...
> Start here for one-off styles. When a component grows variants, promote to a
> [recipe](/docs/recipes/atomic-recipe). The full ladder is in
...
css({ display: 'abc' }) // ❌ will throw error since 'abc' is not part of predefined values of 'display'
css({ pos: 'absolute123' }) // ❌ will throw error since 'absolute123' is not part of predefined values of 'position'
...
> [Environment-specific config](/docs/reference/environment-specific-config) for branching on `process.env`.
