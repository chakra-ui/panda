...
## Share from a preset or config

Put named bags in `theme.viewTransitions` so the team calls `viewTransition('slide')` instead of copying the object.

```ts filename="view-transitions.ts"
...
A static name becomes a string at build time. The import is dropped. Unused names stay out of the CSS.

Use it the same way as the object form: React `share` / `enter` / `exit`, or the class next to Astro's `transition:name`.
...
You can still write `viewTransition({ old, new })` for a one-off. That keeps the hashed `vt_*` class.
...
source. It tree-shakes the same way recipes do, only the bags an app actually imports ship.

A preset can also register the same bags in `theme.viewTransitions` and let apps call `viewTransition('slide')`.
