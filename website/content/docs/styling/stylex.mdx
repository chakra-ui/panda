...
StyleX defines theme values with `stylex.defineVars`, which compiles to real CSS custom properties:
...
styles and no descendant or nested selectors targeting other elements, every rule is scoped to the exact element
`stylex.props()` is spread onto. This is a core design constraint, not a missing feature, it's what lets StyleX
guarantee predictable specificity at scale.
