...
import { definePreset } from '@pandacss/dev/define'
...
Build the preset code. Bundle the definition helper so the published preset does not require `@pandacss/dev` at runtime.

```tsx filename="tsup.config.ts"
