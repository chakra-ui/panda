---
title: Panda CSS 2.0
description: We rewrote the Panda compiler in Rust. Same styling API, a new engine underneath, and the features that rewrite made possible.
date: 2026-08-24
author: ['Segun Adebayo', 'Adebesin Tolulope']
tags: [announcement]
type: release
---
...
- **A new compiler engine written in Rust**, on [Oxc](https://oxc.rs). Same `css()`, recipes, tokens, and JSX props; identical CSS out.
...
**Fixed transform axes** — `rotateX`, `rotateY`, and `rotateZ` now do what they say, `rotate: 'auto'` composes them onto `transform`, and non-uniform `scale` applies rotation after scaling.
...
The full [upgrade guide](/docs/get-started/upgrading-to-v2) is in the docs, and the [playground](https://play.panda-css.com) now runs the same engine in your browser. We built Panda 2.0 because we wanted a faster, smaller engine we could take everywhere: the CLI, your bundler, and the web. Your code doesn't have to know it happened. That's the point.
