...
If a story does not load with SvelteKit and there is output from Storybook that says `[vite] Pre-transform error: Failed to load url /styled-system/css/index.mjs`, add a `viteFinal` section to `.storybook/main.js`:
