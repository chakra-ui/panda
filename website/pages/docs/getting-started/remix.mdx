...
Easily use Panda with Remix with our dedicated integration.
...
Update the `panda.config.ts` file with the following content:
...
> **Note:** We have to explicitly set the `outExtension` to `js` as Remix doesn't support `mjs` files which is the default extension for Panda CSS.
...
```js {14} filename="remix.config.ts"
