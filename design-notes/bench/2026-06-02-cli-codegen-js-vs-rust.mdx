...
caveats:
  ...
  - not equal-work — v2 emits 26 files vs 142 (jsx pass deferred), so `codegen` is partly "less output"; `cssgen` is the
    apples-to-apples row
  - archived benchmark — the v2 CLI prototype measured here has since been merged into packages/cli
---

# CLI: old JS engine vs v2 Rust engine (`codegen` + `cssgen`)

> End-to-end wall-clock comparison of the pre-merge v2 CLI prototype (native NAPI Rust engine) against the legacy JS CLI
> running `codegen` and `cssgen` on the same project. The v2 implementation now lives in `packages/cli`. This is the
> user-facing "how long does the command take" number — the thing someone feels on every save — not a per-workload
> microbenchmark. For the isolated extraction speedup, see
> [2026-05-16 extract-js-vs-rust](./2026-05-16-extract-js-vs-rust.mdx); for the generated-type cost,
> [2026-06-01 generated-types-js-vs-rust](./2026-06-01-generated-types-js-vs-rust.mdx).
...
(`--blur`/`--backdrop-*`/`--gradient-*`/`--breakpoints-*`) for built-in utilities **this config never defines**; v2
emits only what's configured. No used style was missing or different.
...
This note is archived from the pre-merge prototype layout. To re-run this style of benchmark today, use the current v2
CLI path (`node ../../packages/cli/bin.js <command> --cwd . --log-level silent`) and compare it against a published v1
CLI installed outside this workspace.

**[`hyperfine`](https://github.com/sharkdp/hyperfine) example for the current CLI:**

```sh
hyperfine -w 2 -N \
  'node ../../packages/cli/bin.js codegen --cwd . --log-level silent' \
  'node ../../packages/cli/bin.js cssgen --cwd . --log-level silent'
```
