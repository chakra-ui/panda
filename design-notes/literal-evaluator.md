...
| `ObjectExpression`                                                                                                                          | Lenient per-member: an unresolvable property (dynamic value, computed key, getter/method) or spread is _skipped_, keeping the static siblings — `css({ color: 'red', w: props.w })` still folds `color`. Drops to `None` only when **every** member is unresolvable, so a fully-dynamic object emits no phantom style. A spread folds when its argument does: a static object spread overrides earlier keys (last-wins); a **conditional spread** (`...(cond ? a : b)`, or `...(cond && a)` via the logical right-operand rule) keeps each branch's keys separately applicable — they accumulate in a channel separate from the last-wins entries and fold in **after** every property, so a colliding static value (before _or_ after the spread) is unioned, not overwritten. The encoder then emits every branch's value (`{ ...(cond ? {p:'1'} : {p:'2'}), p: '0' }` → `p_0` + `p_1` + `p_2`), mirroring node's order-independent `spreadConditions`. |
| `ArrayExpression`                                                                                                                           | An unresolvable or `undefined` element holds its slot as `null` (like an elision), so later responsive values keep their breakpoint positions (`[undefined, 'red']` → `[null, 'red']`, matching node). An unresolvable _spread_ still drops the whole array (its arity is unknown).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Panda `.raw(...)` calls                                                                                                                     | `css.raw`, `cva.raw`, and pattern raw helpers fold when the callee matches the configured import category. Project parsing can supply the host pattern transform callback so `pattern.raw(...)` folds to the transformed style object instead of the raw props.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
