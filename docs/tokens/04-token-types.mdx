...
Gradient tokens represents a smooth transition between two or more colors. Its value can be defined as a string or a
composite value.
...
const theme = {
  tokens: {
    gradients: {
      primary: {
        value: { type: 'linear', placement: 'to right', colors: ['red', 'blue'] },
      },
    },
  },
}
...
Duration tokens represents the length of time in milliseconds an animation or animation cycle takes to complete. Its
...
const theme = {
  tokens: {
    shadows: {
      ...
      // composite value
      accent: {
        value: { offsetX: 0, offsetY: 4, blurRadius: 4, spreadRadius: 0, color: 'rgba(0, 0, 0, 0.1)' },
      },
