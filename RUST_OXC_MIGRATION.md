...
```txt
Cargo.toml                  # workspace = ["bench", "crates/*", "packages/compiler/crate"]
rust-toolchain.toml
.cargo/config.toml
bench/
crates/
...
packages/compiler/
  package.json              # @pandacss/compiler
...
- `compiler_napi`: the `packages/compiler/crate` cdylib. This is the only crate that should know about NAPI.
...
- `@pandacss/compiler`: native package consumed by the TS packages.
...
| 2.5 | Add `packages/compiler` with NAPI cdylib and TS loader | `@pandacss/compiler` imports in a Node test |
...
- `@pandacss/compiler@alpha` can be installed on macOS, Linux, and Windows.
...
| 3.3 | Expose `scanImports()` from `@pandacss/compiler` | Node test calls Rust and receives JSON |
...
After these, start `OSS-2401` with the Cargo workspace and `@pandacss/compiler` skeleton.
...
- `@pandacss/compiler`: JS wrapper around the native `.node` file.
- `@pandacss/compiler-${platform}`: optional platform-specific native packages generated by NAPI-RS.
...
| Build complexity | Rust and TS release systems differ | Keep `@pandacss/compiler` opt-in until CI/release flow is boring |

## Recommended Implementation Order

1. Add baseline fixtures and benchmark command.
2. Add parser abstraction and `@pandacss/ts-morph` adapter.
3. Add `@pandacss/compiler` native skeleton.
...
- Create `@pandacss/compiler` if `OSS-2401` is complete.
