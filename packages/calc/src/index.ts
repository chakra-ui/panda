export function isCssVar(value: unknown): value is { reference: string } {
  if (typeof value !== 'object') return false;
  return Object.hasOwnProperty.call(value, 'reference');
}
...
function getRef(operand: Operand): string {
  return isCssVar(operand) ? operand.reference : String(operand);
}
