...
ruleTester.run('no-descendant-selectors', {
  valid: [
    // A pseudo-class on the element itself stays scoped.
    { code: withCss("css({ '&:hover': { color: 'red.500' } })"), filename: join(ruleTester.dir, 'self-pseudo.tsx') },
    // A condition is the blessed cross-element form.
    { code: withCss("css({ _hover: { color: 'red.500' } })"), filename: join(ruleTester.dir, 'condition.tsx') },
    // Pseudo-elements belong to the element.
    {
      code: withCss("css({ '&::before': { color: 'red.500' } })"),
      filename: join(ruleTester.dir, 'pseudo-element.tsx'),
    },
    // An attribute selector on self, even with a space inside the brackets.
    {
      code: withCss(`css({ '&[data-state="open item"]': { color: 'red.500' } })`),
      filename: join(ruleTester.dir, 'attr-self.tsx'),
    },
    // A class added to the same element is still self-targeting.
    { code: withCss("css({ '&.active': { color: 'red.500' } })"), filename: join(ruleTester.dir, 'self-class.tsx') },
    // :not() arguments can contain combinator characters without leaving the element.
    {
      code: withCss("css({ '&:not([data-x])': { color: 'red.500' } })"),
      filename: join(ruleTester.dir, 'not-arg.tsx'),
    },
    // `.foo&` is a compound with `&`: the same element, written backwards.
    { code: withCss("css({ '.foo&': { color: 'red.500' } })"), filename: join(ruleTester.dir, 'compound-prefix.tsx') },
    // :is() may hold full complex selectors; combinators inside its parens stay shielded.
    {
      ...
      filename: join(ruleTester.dir, 'is-complex-arg.tsx'),
    },
    // :has() looks at descendants but styles the element itself.
    {
      code: withCss("css({ '&:has(> img + figcaption)': { color: 'red.500' } })"),
      filename: join(ruleTester.dir, 'has-arg.tsx'),
    },
    // An attribute value may contain a quoted bracket without corrupting depth tracking.
    {
      ...
      filename: join(ruleTester.dir, 'attr-quoted-bracket.tsx'),
    },
    // An attribute value may contain a comma; it is not a selector-list separator.
    {
      code: withCss(`css({ '&[data-tags="a,b"]': { color: 'red.500' } })`),
      filename: join(ruleTester.dir, 'attr-quoted-comma.tsx'),
    },
    // An escaped space in a class name is part of the name, not a combinator.
    {
      code: withCss("css({ '&.foo\\\\ bar': { color: 'red.500' } })"),
      filename: join(ruleTester.dir, 'escaped-space.tsx'),
    },
    // `:is(&)` wraps the nesting selector without leaving the element.
    {
      code: withCss("css({ ':is(&):hover': { color: 'red.500' } })"),
      filename: join(ruleTester.dir, 'is-wrapped-amp.tsx'),
    },
  ],
  invalid: [
    ...
    // Selecting a descendant of :is() output leaves the element too.
    {
      ...
      filename: join(ruleTester.dir, 'is-then-descendant.tsx'),
