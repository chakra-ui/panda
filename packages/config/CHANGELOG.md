...
- Clearer `designSystem` errors for bad manifests, missing exports, unsupported protocols, and duplicate
  names.
...
- `panda lib` resolves `npm:` peer aliases (like `npm:@pandacss/dev@^3.0.0`) into a real `panda` range in the
  manifest, same as `workspace:` and `catalog:`.
...
- Fix config loading failing with `The argument 'filename' must be a file URL … Received 'data:…'` when the
