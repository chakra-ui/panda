import type { Compiler, FileInspectionResult } from '@pandacss/compiler-shared'

export interface FileInspectionCacheEntry {
  sourceKey: string
  result: FileInspectionResult
}

/** Caches `compiler.inspectFile()` results per `(compiler, path)`, invalidated by source content. */
export class FileInspector {
  #entries = new WeakMap<Compiler, Map<string, FileInspectionCacheEntry>>()
