...
const props = defineProps<{ themes: string[]; theme: string }>();
const emit = defineEmits<{ change: [value: string] }>();

const cls = select({ orientation: "vertical" });
...
const collection = computed(() =>
  createListCollection({
    items: [
      { label: "Default", value: "" },
      ...props.themes.map((name) => ({ label: title(name), value: name })),
    ],
  }),
);
</script>
...
<template>
  ...
  >
    <Select.Label :class="cls.label">Theme</Select.Label>
    <Select.Control>
