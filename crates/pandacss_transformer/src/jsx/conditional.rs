...
use crate::ternary::{ParsedTernary, parse_top_level_ternary};
...
pub(super) fn class_expression_for_jsx_data(
    ...
    if sites.len() == 1 {
        return single_site_expression(project, jsx, entries, &sites[0]);
    }

    let parts: Option<Vec<String>> = sites
        .iter()
        .map(|site| single_site_expression(project, jsx, entries, site))
        .collect();
    let parts = parts?;
    Some(
        parts
...
pub(super) fn class_expression_for_runtime_props(
    ...
    class_expression_for_jsx_data(project, jsx, &opening)
}
...
fn single_site_expression(
    ...
    for branch in &site.branches {
        ...
        let classes = class_names_for_branch_literal(project, jsx, &Literal::Object(next))?;
...
fn class_names_for_branch_literal(
    ...
    Some(project.class_names_for_jsx_usage(&branch_jsx)?.join(" "))
}
...
mod tests {
    ...
    use crate::ternary::parse_top_level_ternary;
