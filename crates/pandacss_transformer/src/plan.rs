...
pub(crate) fn build_plan(
        ...
        match call.category {
            MatchCategory::Css if targets.css_enabled() => match call.name.as_str() {
                "cva" => {
                    ...
                    if let Some(rewrite) =
                        crate::recipe_inline::rewrite_for_cva_call(project, call.span, &call.data)
                    {
                ...
                "sva" => {
                    ...
                    if let Some(rewrite) =
                        crate::recipe_inline::rewrite_for_sva_call(project, call.span, &call.data)
                    {
                        plan.rewrites.push(rewrite);
                ...
                _ => match resolve::rewrite_for_css_call(project, source, call.span, &call.data) {
                    ...
                    None if crate::css_conditional::args_need_conditional_rewrite(
            ...
            MatchCategory::Jsx if targets.jsx_enabled() => {
                if let Some(rewrite) = crate::styled::rewrite_for_styled_call(project, source, call)
                {
                    plan.rewrites.push(rewrite);
                    plan.helper.needs_cva = true;
                }
    ...
    if targets.jsx_enabled() {
        for jsx in &extracted.jsx {
            plan.rewrites.extend(crate::jsx::rewrites_for_jsx_element(
                project,
                source,
                jsx,
                options.helper_cx,
