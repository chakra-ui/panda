...
pub(super) fn rewrite(
    ...
    for fragment in &fragments {
        insert_disjoint(&mut keys, style_keys(system, &fragment.tree)?)?;
        if let Some(runtime) = &fragment.runtime {
            preserved.push(runtime.span);
            printed.push(None);
        } else {
            let (expr, spans) =
                style_lower::lower_css_args(system, source, &[Some(fragment.tree.clone())])?;
    ...
    {
        print_pure_split(system, source, callee, &finite, &fragments)?
    } else {
