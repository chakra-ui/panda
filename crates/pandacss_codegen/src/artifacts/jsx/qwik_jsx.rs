...
pub(super) fn module(factory: &str, component: &str, upper: &str) -> Module {
    let runtime = FACTORY_RUNTIME
        .replace(
            ...
            "const __COMPONENT__ = function __COMPONENT__(props) {",
        )
        .replace(
...
const FACTORY_RUNTIME: &str = r"function styledFn(BaseComponent, recipeOrConfig = {}, options = {}) {
  ...
  const __COMPONENT__ = /* @__PURE__ */ forwardRef(function __COMPONENT__(props, ref) {
    ...
    let className
    if (unstyled) {
      className = cx(hasStyles && serializeSplitStyles(propStyles, cssStyles), combinedProps.class, combinedProps.className)
