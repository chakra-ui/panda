//! The `helpers` artifact: the shared runtime functions the other artifacts
//! import (`memo`, `splitProps`, css normalize/merge, object utils, `cx`'s
//! `uniq`, …). [`module`] fixes their emit order; the submodules group them by
//! concern (`css`, `object`, `misc`, `split_props`).

mod css;
mod misc;
mod object;
mod split_props;
mod token;

use crate::{
    Artifact, ArtifactFile, ArtifactId, CodegenContext, DependencySet, ExportDecl, Item, ItemNode,
    Module, RuntimeImport,
    graph::{GenerateOptions, emit_module_files},
};

#[must_use]
pub fn module(ctx: CodegenContext<'_>) -> Module {
    Module::new()
        .with_item(object::is_object())
        .with_item(object::has_own())
        .with_item(misc::is_base_condition())
        .with_item(misc::filter_base_conditions())
        .with_item(misc::to_hash())
        .with_item(token::to_css_var(ctx))
        .with_item(token::color_mix())
        .with_item(object::compact())
        .with_item(object::with_defaults())
        .with_item(object::to_variant_map())
        .with_item(object::compound_variant_matches())
        .with_item(object::get_compound_variant_class_names())
        .with_item(object::get_compound_variant_css())
        .with_item(object::get_slot_compound_variant())
        .with_item(object::get_slot_recipes())
        .with_item(css::to_responsive_object())
        .with_item(object::walk_object())
        .with_item(object::map_object())
        .with_item(css::normalize_style_object())
        .with_item(misc::flat_hash_or_null())
        .with_item(misc::flat_args_equal())
        .with_item(misc::memo())
        .with_item(misc::weak_memo())
        .with_item(object::merge_props())
        .with_item(css::sanitize_style_value())
        .with_item(css::resolve_style_args())
        .with_item(css::create_serialize_css())
        .with_item(css::create_merge_css())
        .with_item(css::create_serialize_css_args())
        .with_item(css::create_assign_css())
        .with_item(css::hypenate_property_regexes())
        .with_item(css::hypenate_property())
        .with_item(split_props::split_props_key_map_cache())
        .with_item(split_props::split_props_key_map())
        .with_item(split_props::copy_split_prop())
        .with_item(split_props::split_props())
        .with_item(misc::normalize_html_props())
        .with_item(misc::normalize_html_props_types())
        .with_item(misc::uniq())
        .with_item(misc::without_space())
        .with_item(misc::is_important())
        .with_item(misc::without_important())
}

#[must_use]
pub fn files(
    ctx: CodegenContext<'_>,
    options: GenerateOptions,
    dependencies: DependencySet,
) -> Vec<ArtifactFile> {
    // Same shape as virtualized `css/index`: keep a local entry that star-re-exports the DS
    // module so `import { memo } from '../styled-system/helpers'` still resolves.
    let body = if let Some(overlay) = ctx.overlay
        && ctx.virtualizes(RuntimeImport::Helpers)
    {
        Module::new().with_item(Item::both(ItemNode::Export(ExportDecl::Star {
            source: overlay.helpers.clone(),
        })))
    } else {
        module(ctx)
    };

    emit_module_files(
        "helpers",
        &body,
        options.format,
        false,
        options.import_extensions,
        dependencies,
    )
}

#[must_use]
pub fn generate(
    ctx: CodegenContext<'_>,
    options: GenerateOptions,
    dependencies: DependencySet,
) -> Artifact {
    Artifact {
        id: ArtifactId::Helpers,
        dependencies,
        files: files(ctx, options, dependencies),
    }
}
