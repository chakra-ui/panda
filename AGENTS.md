...
/packages/          # Published packages
  /cli/            # @pandacss/cli — the `panda` CLI
  /config/         # @pandacss/config — config load / bundle / snapshot
  /dev/            # @pandacss/dev — config helpers plus node/cli facade subpaths
...
- Rust: integration tests live in `crates/<name>/tests/`; see `design-notes/rust-testing.md`
...
@pandacss/dev
  ...
  ├─ /node (config loading + compiler APIs)
  └─ /cli (command runners)
