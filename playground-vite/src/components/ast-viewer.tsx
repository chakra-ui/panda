...
const resultType = cva({
  ...
  variants: {
    type: {
      object: {
        bg: { base: 'gray.100', _dark: '#FFFFFF08' },
        color: { base: 'gray.700', _dark: 'white' },
      },
      cva: {
        bg: { base: 'gray.300', _dark: '#FFFFFF12' },
        color: { base: 'gray.700', _dark: 'white' },
      },
      sva: {
        bg: { base: 'gray.300', _dark: '#FFFFFF12' },
      ...
      'jsx-factory': {
        bg: { base: 'blue.100', _dark: 'blue.300' },
        color: { _dark: 'black' },
      },
      pattern: {
        bg: { base: 'indigo.400', _dark: 'indigo.500' },
        color: 'white',
      },
      'jsx-pattern': {
        bg: { base: 'indigo.400', _dark: 'indigo.500' },
      ...
      recipe: {
        bg: { base: 'yellow.300', _dark: 'yellow.500' },
        color: { _dark: 'black' },
      },
      'jsx-recipe': {
        bg: { base: 'yellow.300', _dark: 'yellow.500' },
...
const ResultItemRow = (props: { result: ResultItem }) => {
  ...
  return (
    <Stack px='6'>
      <styled.div className={hstack()}>
        <span className={resultType({ type: result.type })}>{result.type}</span>{' '}
        <span className={resultType({ name: result.name as 'cva' | 'css' })}>
          {result.name}
        </span>
        <styled.span ml='auto'>(l{getReportRange(result)})</styled.span>
      </styled.div>
      <JsonViewer
        className={css({
